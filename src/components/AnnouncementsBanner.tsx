  ...
  return (
    <div className="space-y-3">
      {visible.map((announcement) => (
        ...
        >
          <Megaphone
            className={`h-5 w-5 mt-0.5 flex-shrink-0 ${announcement.priority === 'high' ? 'text-red-600' : 'text-blue-600'}`}
          />
          ...
          <button onClick={() => dismiss(announcement.id)} className="text-gray-400 hover:text-gray-600 flex-shrink-0">
            <X className="h-4 w-4" />
          </button>
        </div>
      ))}
