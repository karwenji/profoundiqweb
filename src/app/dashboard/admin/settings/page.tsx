  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = [
        ...
        { id: 'webhook', ref: webhookRef },
      ]
      
      for (const section of sections) {
        const element = (section.ref as React.RefObject<HTMLDivElement>).current
        if (element) {
            ...
            <Card className="sticky top-6">
              <CardContent className="p-4">
                <nav className="space-y-2">
                  <button
                    onClick={() => scrollToSection('general', generalRef)}
                  ...
                  <button
                    onClick={() => scrollToSection('payment', paymentRef)}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-left ${
